...
st.markdown('# Regex + Tokenization')
...
st.markdown("""
    ...
    of tokenization like this can be implemented with ease using a library like `nltk` and it's utilities `sent_tokenize`
    and `word_tokenize`. However, you can also use regex and `regexp_tokenize` to create custom patterns for tokenization.
""")
...
example_text = st.text_area('Enter some example text:', 'This is some example text with a #tag.')
